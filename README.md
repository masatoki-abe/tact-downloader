...
対象は名古屋大学 TACT に限定しています。THERS 認証の自動操作、`SEMESTER_PATTERNS` の学期表記（`春1期`〜`秋3期` などのクォーター制）、ダウンロード先のフォルダ構成はいずれも名古屋大学の運用に合わせたもので、他大学の Sakai 環境では動作保証しません。

対応Pythonバージョンは **Python 3.10以上** です。
