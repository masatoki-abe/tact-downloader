...
`--dry-run` は認証、TACT APIへのサイト・リソース一覧取得、ローカルの既存ファイル確認を行いますが、vault内のファイルやディレクトリは作成・更新しません。保存Cookieの状態によっては、Cookieファイルの権限補正、作成、更新が発生する場合があります。既存ファイルはスキップとして表示され、`--force --dry-run` ではダウンロード予定として表示されます。
